...
xgb_model <- xgb.train(params=xgb_params, xgb.DMatrix(x_train, label=y_train, missing=NaN), nrounds=best_nrounds)

y_train_pred <- predict(xgb_model, xgb.DMatrix(x_train, missing=NaN))
y_test_pred <- predict(xgb_model, xgb.DMatrix(x_test, missing=NaN))
y_validation_pred <- predict(xgb_model, xgb.DMatrix(x_validation, missing=NaN))

rmse <- function(actual, predicted) {
    sqrt(mean((actual-predicted)^2))
}
print(sprintf("Train RMLSE is %f", rmse(y_train, y_train_pred)))
print(sprintf("Test RMLSE is %f", rmse(y_test, y_test_pred)))
print(sprintf("Validation RMLSE is %f", rmse(y_validation, y_validation_pred)))
```

## Final prediction using all data

```{r}
x_all <- rbind(x_train, x_test, x_validation)
y_all <- c(y_train, y_test, y_validation)
